<?php
  ...
  if(empty($taskassignto) or empty($taskcontent) or empty($supervisor)){
        $msg = "<div class='alert alert-success'>Fill all fields please </div>";
        
  }else{
      $queryTask = mysqli_query($conn, "INSERT INTO sys_task (assigned_to, task_name, supervisor) 
      VALUES ('$taskassignto', '$taskcontent', '$supervisor')");
      if($queryTask){
                                    ...
                                    <form action="" method="POST">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select type="text" class="form-control" name="supervisor">
                                                <?php
                                                    $querySuper = mysqli_query($conn, "SELECT * FROM sys_task");
                                                    while($rowsSuper = mysqli_fetch_array($querySuper)){
                                                ?>

                                                <option value="<?php echo $rowsSuper['task_id'];?>">
                                                    <?php echo $rowsSuper['supervisor'];?></option>
